...
from backend.schemas.student import StudentCreate, StudentResponse, StudentLogin
...
async def login(login_data: StudentLogin, db: AsyncSession = Depends(get_db)):
    """Authenticate student and return JWT token."""
    from loguru import logger
    logger.info(f"Login attempt for roll_no: {login_data.roll_no}")
    
    from sqlalchemy import or_, func
    username_normalized = login_data.roll_no.strip()
    result = await db.execute(
        select(Student).where(
            or_(
                func.lower(Student.roll_no) == username_normalized.lower(),
                func.lower(Student.email) == username_normalized.lower()
            )
        )
    )
