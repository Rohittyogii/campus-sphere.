...
class StudentCafeFavorite(Base):
    """Represents a student's favorite cafe item."""
    __tablename__ = "student_cafe_favorites"

    id = Column(Integer, primary_key=True, index=True)
    student_roll_no = Column(String(30), nullable=False, index=True)
    item_name = Column(String(200), nullable=False)

    def __repr__(self):
        return f"<Favorite {self.item_name} for {self.student_roll_no}>"
