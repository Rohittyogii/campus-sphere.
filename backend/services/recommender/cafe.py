...
async def recommend_cafe(db: AsyncSession, student: Student) -> dict:
    ...
    # ─────────────────────────────────────────────
    # 3. Trending Items (Global popularity simulation)
    # ─────────────────────────────────────────────
    # We'll pick 5 items that are available, prioritizing variety across categories
    trending_items = []
    seen_cats = set()
    for item in sorted(available, key=lambda x: x.price, reverse=True): # Pick premium items for trending
        if item.category not in seen_cats and len(trending_items) < 5:
    ...
    # Fill up if less than 5
    if len(trending_items) < 5:
        for item in available:
            if item.itemid not in [i["itemid"] for i in trending_items] and len(trending_items) < 5:
                trending_items.append({
                    "itemid": item.itemid,
                    "item_name": item.item_name,
                    "category": item.category,
